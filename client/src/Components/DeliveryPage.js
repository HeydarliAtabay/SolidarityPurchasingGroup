...
function DeliveryPage(props) {
  ...
  useEffect(() => {
    ...
    if (props.mail) getDelivererByEmail();
  }, [props.mail]);
  ...
  return (
    <>
      ...
      <h1 className="mt-3 text-center mb-3">
        {avaiableOrders.length != 0 ? (
          <>
            There are {avaiableOrders.length} available order(s) in{' '}
            {delivererData.city}!
          </>
        ) : (
          <>There is no order in your city</>
        )}
      </h1>
      <h1></h1>
      {/*
            ...
            return (
              <>
                <tr key={o.id} className="m-5">
                  ...
                  <td>{o.zipcode}</td>
                  <td>{o.city}</td>
                  <td style={{ color: '#4682B4', fontWeight: '700' }}>
                    {o.state}
                  </td>
                  <td>
                    {
