...
function ManagerFarmers(props) {
  ...
  const [refreshPendingApplication, setRefreshPendingApplications] = useState(
    props.pendingOnly
  );

  const [acceptedApplications, setAcceptedApplications] = useState([]);
  const [refreshAcceptedApplication, setRefreshAcceptedApplications] = useState(
    props.acceptedOnly
  );
  ...
  return (
    <div className="row">
      ...
      <h5 className="d-block mx-auto mb-5 text-center text-muted">
        {props.pendingOnly &&
          'Examine the applications below and approve or reject them'}
        {props.acceptedOnly &&
          'Browse and see in detail previously accepted applications'}
      </h5>
      {operationAlert && (
        <div className="row">
              ...
              pendingApplications.map((a) => (
                <Card key={a.id} className="mb-4 shadow" border="secondary">
                  <Card.Header as="h4">
                    <div className="container">
                      <div className="row">
                        <div className="col-lg-6 text-start mt-2 mb-2">
                  ...
                  <Card.Body>
                    <Card.Title className="ms-5">
                      {capitalizeEachFirstLetter(a.name) +
                        ' ' +
                        capitalizeEachFirstLetter(a.surname)}
                    </Card.Title>
                    ...
                    <div className="d-block text-center">
                      <Button
                        variant="primary"
                        onClick={() => setInspectApplication(a)}
                      >
                        {viewApplicationIcon} View application
                      </Button>
                    </div>
                  </Card.Body>
                  <Card.Footer className="text-muted">
                    Application received {calculateElapsedTime(a.date)}
                  </Card.Footer>
                </Card>
              ))}
            {!loading &&
                      ...
                      <div className="row">
                        ...
                        <div className="col-lg-6 text-end my-auto">
                          <h5 className="my-auto">
                            <Badge
                              bg={
                                a.status === 'accepted' ? 'success' : 'danger'
                              }
                              text="dark"
                            >
                              Status: {a.status}
...
function ApplicationInspector(props) {
  const [countryCode] = useState(
    Country.getAllCountries().find((c) => c.name === props.application.country)
      .isoCode
  );
  const [regionCode] = useState(
    State.getStatesOfCountry(
      Country.getAllCountries().find(
        (c) => c.name === props.application.country
      ).isoCode
    ).find((r) => r.name === props.application.region).isoCode
  );
    ...
    const update = async () => {
      if (updateApplication === 'accept') {
        ...
        if (response) {
          props.setAlert({
            type: 'success',
            msg: 'The farmer application has been successfully accepted.',
          });
        } else {
          props.setAlert({
            type: 'danger',
            msg: 'Oops! An error occurred and the application status could not be changed. Please try again later.',
          });
        }
      } else {
        const response = await API.rejectFarmerApplication(
          props.application.id
        );
        if (response) {
          props.setAlert({
            type: 'success',
            msg: 'The farmer application has been successfully rejected.',
          });
        } else {
          props.setAlert({
            type: 'danger',
          ...
          <div className="col-md-6">
            ...
            >
              <Form.Control
                ...
                value={props.application.surname}
                readOnly
              />
            </FloatingLabel>
          </div>
          ...
          <div className="col-md-9">
            ...
            >
              <Form.Control
                ...
                value={props.application.address}
                readOnly
              />
            </FloatingLabel>
          </div>
        ...
        {props.pendingOnly && (
          <div className="subBtn">
            <Button
              ...
              size="lg"
              className="mb-2"
              onClick={() => setUpdateApplication('reject')}
            >
              Reject application
            </Button>
            <Button
